...
Configuring Git('--global'):
    -Before the work begin, you need to tell Git who you are! This information is attached to everything you do so people know who made the changes.
        ...
        -Commands:
            ...
            Get: 'git config get [scope] <key>'*(3)*
                Example: 'git config get init.defaultBranch'
            Unset: 'git config unset [scope] <key>' (The "Eraser").
            if scope isn't specified git will use the default one ('--local')*(4)*.
        -Keys:
    ...
    *(2):The ~ (tilde) is a shortcut that means "my home folder," which is where Git looks for your default identity.*
    *(3):You can't just ask for <key>; you must ask for <section>.<key>**(1)**, git follows a strict format, It is like looking for a specific word in a dictionary. You don't just look for "Definition"; you look for "Bear.Definition" so Git knows exactly which section to check.**(2)***
        ...
        -**(2):Existence Rule: A section only exists as long as it has at least one key. If you unset the last key, it will leave a empty [section] header behind in the .git/config file. (To remove it, go to 'Configuring Git ('--Local')/Commands/Remove/Chapter Eraser').**
    *(4):The Safety Rule: You must be "inside" a Git project folder to use or see 'Local' settings. If you try to 'set' a local key while standing outside a project, Git will get confused and tell you: "fatal: not in a git directory".*
...
*Context Summary: Git Apprentice Reference Guide
    ...
    Key Analogies & Frameworks:
        ...
        The Config Notebook:
            -The Inside Cover (--global): Your identity and settings for all projects.
            -The Sticky Note (--local): Project-specific settings that override the inside cover.
